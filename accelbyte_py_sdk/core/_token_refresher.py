...
class TokenRefresher:
    def __init__(
        self, refresh_func: Callable, *args, refresh_rate: float = 0.8, **kwargs
    ):
    ...
    def refresh(self, token_repo: TokenRepository):
        if refresh_token := token_repo.get_refresh_token():
            ...
            self._run_sync(
                self.refresh_func, *self.refresh_func_args, **self.refresh_func_kwargs
            )
    ...
    def try_refresh(self, token_repo: TokenRepository) -> bool:
        with self._lock:
            if (
                token_repo is None
                or not token_repo.has_token()
                or not token_repo.has_token_expired(multiplier=self.refresh_rate)
            ):
                return False
    ...
    def _refresh_with_token(self, refresh_token: str):
        ...
        from accelbyte_py_sdk.api.iam.operations.o_auth2_0 import (
