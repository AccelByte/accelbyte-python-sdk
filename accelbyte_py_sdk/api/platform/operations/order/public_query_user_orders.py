...
class PublicQueryUserOrders(Operation):
    ...
    def is_valid(self) -> bool:
        ...
        if not hasattr(self, "user_id") or self.user_id is None:
            return False
        # enum checks
        if hasattr(self, "status") and self.status is not None and self.status not in PublicQueryUserOrders.get_enum_map()["status"]:
            return False
        # pattern checks
        return True
    ...
    # noinspection PyMethodMayBeStatic
    def parse_response(self, code: int, content_type: str, content: Any) -> Tuple[Union[None, OrderPagingSlicedResult], Union[None, HttpResponse]]:
    ...
    @classmethod
    def create_from_dict(cls, dict_: dict, include_empty: bool = False) -> PublicQueryUserOrders:
        instance = cls()
