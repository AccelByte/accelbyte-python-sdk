...
class PaymentUrlCreate(Model):
    ...
    def is_valid(self) -> bool:
        ...
        if not hasattr(self, "payment_provider") or self.payment_provider is None:
            return False
        # enum checks
        if hasattr(self, "payment_provider") and self.payment_provider is not None and self.payment_provider not in PaymentUrlCreate.get_enum_map()["paymentProvider"]:
            return False
        # pattern checks
        return True
    ...
    @classmethod
    def create_from_any(cls, any_: any, include_empty: bool = False, many: bool = False) -> Union[PaymentUrlCreate, List[PaymentUrlCreate]]:
        if many:
