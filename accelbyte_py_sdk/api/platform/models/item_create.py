...
class ItemCreate(Model):
    ...
    def is_valid(self) -> bool:
        ...
        if not hasattr(self, "status") or self.status is None:
            return False
        # enum checks
        if hasattr(self, "entitlement_type") and self.entitlement_type is not None and self.entitlement_type not in ItemCreate.get_enum_map()["entitlementType"]:
            return False
        if hasattr(self, "item_type") and self.item_type is not None and self.item_type not in ItemCreate.get_enum_map()["itemType"]:
            return False
        if hasattr(self, "status") and self.status is not None and self.status not in ItemCreate.get_enum_map()["status"]:
            return False
        if hasattr(self, "app_type") and self.app_type is not None and self.app_type not in ItemCreate.get_enum_map()["appType"]:
            return False
        if hasattr(self, "season_type") and self.season_type is not None and self.season_type not in ItemCreate.get_enum_map()["seasonType"]:
            return False
        # pattern checks
        return True
    ...
    @classmethod
    def create_from_any(cls, any_: any, include_empty: bool = False, many: bool = False) -> Union[ItemCreate, List[ItemCreate]]:
        if many:
