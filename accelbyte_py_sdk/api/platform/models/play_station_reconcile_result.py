...
class PlayStationReconcileResult(Model):
    ...
    def is_valid(self) -> bool:
        ...
        # required checks
        # enum checks
        if hasattr(self, "status") and self.status is not None and self.status not in PlayStationReconcileResult.get_enum_map()["status"]:
            return False
        # pattern checks
        return True
    ...
    @classmethod
    def create_from_any(cls, any_: any, include_empty: bool = False, many: bool = False) -> Union[PlayStationReconcileResult, List[PlayStationReconcileResult]]:
        if many:
