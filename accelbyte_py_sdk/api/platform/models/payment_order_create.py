...
class PaymentOrderCreate(Model):
    ...
    def is_valid(self) -> bool:
        ...
        if not hasattr(self, "title") or self.title is None:
            return False
        # enum checks
        if hasattr(self, "item_type") and self.item_type is not None and self.item_type not in PaymentOrderCreate.get_enum_map()["itemType"]:
            return False
        # pattern checks
    ...
    @classmethod
    def create_from_any(cls, any_: any, include_empty: bool = False, many: bool = False) -> Union[PaymentOrderCreate, List[PaymentOrderCreate]]:
        if many:
