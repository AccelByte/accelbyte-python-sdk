...
class PopulatedItemInfo(Model):
    ...
    def with_season_type(self, value: str) -> PopulatedItemInfo:
    ...
    def is_valid(self) -> bool:
        ...
        if not hasattr(self, "updated_at") or self.updated_at is None:
            return False
        # enum checks
        if hasattr(self, "entitlement_type") and self.entitlement_type is not None and self.entitlement_type not in PopulatedItemInfo.get_enum_map()["entitlementType"]:
            return False
        if hasattr(self, "item_type") and self.item_type is not None and self.item_type not in PopulatedItemInfo.get_enum_map()["itemType"]:
            return False
        if hasattr(self, "status") and self.status is not None and self.status not in PopulatedItemInfo.get_enum_map()["status"]:
            return False
        if hasattr(self, "app_type") and self.app_type is not None and self.app_type not in PopulatedItemInfo.get_enum_map()["appType"]:
            return False
        if hasattr(self, "season_type") and self.season_type is not None and self.season_type not in PopulatedItemInfo.get_enum_map()["seasonType"]:
            return False
        # pattern checks
        return True
    ...
    @classmethod
    def create_many_from_dict(cls, dict_: dict, include_empty: bool = False) -> Dict[str, PopulatedItemInfo]:
        return {k: cls.create_from_dict(v, include_empty=include_empty) for k, v in dict_} if dict_ else {}

    @classmethod
    def create_many_from_list(cls, list_: list, include_empty: bool = False) -> List[PopulatedItemInfo]:
        return [cls.create_from_dict(i, include_empty=include_empty) for i in list_] if list_ else []

    @classmethod
    def create_from_any(cls, any_: any, include_empty: bool = False, many: bool = False) -> Union[PopulatedItemInfo, List[PopulatedItemInfo]]:
        if many:
