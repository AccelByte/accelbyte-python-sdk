...
class ListIncomingFriendsResponse(WebSocketMessage):
    ...
    @classmethod
    def create_from_wsm(cls, wsm: str, is_strict: bool = False) -> ListIncomingFriendsResponse:
        instance = cls()
        if not wsm:
            return instance
        ...
        for line in lines[2:]:
            ...
            if (not is_strict and name.casefold() == "code".casefold()) or (name == "code"):
                instance.code = value
                continue
            if (not is_strict and name.casefold() == "userIds".casefold()) or (name == "userIds"):
                instance.user_ids = [str(i) for i in value.removeprefix("[").removesuffix("]").split(",")]
                continue
            if is_strict:
