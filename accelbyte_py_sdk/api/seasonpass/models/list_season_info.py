...
class ListSeasonInfo(Model):
    ...
    def is_valid(self) -> bool:
        ...
        if not hasattr(self, "updated_at") or self.updated_at is None:
            return False
        # enum checks
        if hasattr(self, "status") and self.status is not None and self.status not in ListSeasonInfo.get_enum_map()["status"]:
            return False
        # pattern checks
        return True
    ...
    @classmethod
    def create_many_from_dict(cls, dict_: dict, include_empty: bool = False) -> Dict[str, ListSeasonInfo]:
        return {k: cls.create_from_dict(v, include_empty=include_empty) for k, v in dict_} if dict_ else {}

    @classmethod
    def create_many_from_list(cls, list_: list, include_empty: bool = False) -> List[ListSeasonInfo]:
        return [cls.create_from_dict(i, include_empty=include_empty) for i in list_] if list_ else []

    @classmethod
    def create_from_any(cls, any_: any, include_empty: bool = False, many: bool = False) -> Union[ListSeasonInfo, List[ListSeasonInfo]]:
        if many:
