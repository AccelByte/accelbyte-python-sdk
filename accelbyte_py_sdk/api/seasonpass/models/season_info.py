...
class SeasonInfo(Model):
    ...
    def is_valid(self) -> bool:
        ...
        if not hasattr(self, "end") or self.end is None:
            return False
        if not hasattr(self, "excess_strategy") or self.excess_strategy is None:
            return False
        ...
        if not hasattr(self, "tier_item_id") or self.tier_item_id is None:
            return False
        if not hasattr(self, "tier_item_name") or self.tier_item_name is None:
            return False
        if not hasattr(self, "updated_at") or self.updated_at is None:
            return False
        # enum checks
        if hasattr(self, "status") and self.status is not None and self.status not in SeasonInfo.get_enum_map()["status"]:
            return False
        # pattern checks
        return True
