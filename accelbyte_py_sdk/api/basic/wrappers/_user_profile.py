...
@same_doc_as(CreateMyProfile)
async def create_my_profile_async(body: Optional[UserProfilePrivateCreate] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(DeleteUserProfile)
async def delete_user_profile_async(user_id: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(GetCustomAttributesInfo)
async def get_custom_attributes_info_async(user_id: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(GetMyProfileInfo)
async def get_my_profile_info_async(namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(GetMyZipCode)
async def get_my_zip_code_async(namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(GetPrivateCustomAttributesInfo)
async def get_private_custom_attributes_info_async(user_id: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(GetUserProfileInfo)
async def get_user_profile_info_async(user_id: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicCreateUserProfile)
async def public_create_user_profile_async(user_id: str, body: Optional[UserProfileCreate] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicGetCustomAttributesInfo)
async def public_get_custom_attributes_info_async(user_id: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicGetUserProfileInfo)
async def public_get_user_profile_info_async(user_id: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicGetUserProfilePublicInfo)
async def public_get_user_profile_public_info_async(user_id: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicGetUserProfilePublicInfoByIds)
async def public_get_user_profile_public_info_by_ids_async(user_ids: str, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicUpdateCustomAttributesPartially)
async def public_update_custom_attributes_partially_async(user_id: str, body: Optional[Dict[str, Any]] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicUpdateUserProfile)
async def public_update_user_profile_async(user_id: str, body: Optional[UserProfileUpdate] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(PublicUpdateUserProfileStatus)
async def public_update_user_profile_status_async(user_id: str, body: Optional[UserProfileStatusUpdate] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(UpdateCustomAttributesPartially)
async def update_custom_attributes_partially_async(user_id: str, body: Optional[Dict[str, Any]] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(UpdateMyProfile)
async def update_my_profile_async(body: Optional[UserProfilePrivateUpdate] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(UpdateMyZipCode)
async def update_my_zip_code_async(user_zip_code_update: UserZipCodeUpdate, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(UpdatePrivateCustomAttributesPartially)
async def update_private_custom_attributes_partially_async(user_id: str, body: Optional[Dict[str, Any]] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(UpdateUserProfile)
async def update_user_profile_async(user_id: str, body: Optional[UserProfileAdmin] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
...
@same_doc_as(UpdateUserProfileStatus)
async def update_user_profile_status_async(user_id: str, body: Optional[UserProfileStatusUpdate] = None, namespace: Optional[str] = None, x_additional_headers: Optional[Dict[str, str]] = None):
    if namespace is None:
        namespace, error = get_services_namespace()
        if error:
            return None, error
