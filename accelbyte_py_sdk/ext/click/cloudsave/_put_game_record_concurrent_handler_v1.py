import json
from typing import Optional

import click

from .._utils import login_as as login_as_internal
from ....api.cloudsave import put_game_record_concurrent_handler_v1 as put_game_record_concurrent_handler_v1_internal
from ....api.cloudsave.models import ModelsConcurrentRecordRequest
from ....api.cloudsave.models import ModelsResponseError


@click.command()
@click.argument("body", type=str)
@click.argument("key", type=str)
@click.option("--namespace", type=str)
@click.option("--doc", type=bool)
@click.option("--login_as", type=click.Choice(["client", "user"], case_sensitive=False))
def put_game_record_concurrent_handler_v1(
        body: str,
        key: str,
        namespace: Optional[str] = None,
        doc: Optional[bool] = None,
        login_as: Optional[str] = None,
):
    login_as_internal(login_as)
    if doc:
        click.echo(put_game_record_concurrent_handler_v1_internal.__doc__)
    try:
        body_json = json.loads(body)
        body = ModelsConcurrentRecordRequest.create_from_dict(body_json)
    except ValueError as e:
        raise Exception(f"Invalid JSON for 'body'. {str(e)}")
    result, error = put_game_record_concurrent_handler_v1_internal(
        body=body,
        key=key,
        namespace=namespace,
    )
    if error:
        raise Exception(f"putGameRecordConcurrentHandlerV1 failed: {str(error)}")
    click.echo("putGameRecordConcurrentHandlerV1 success")
