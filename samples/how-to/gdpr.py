...
def example_submit_user_account_deletion():
    ...
    result, error = admin_submit_user_account_deletion_request(user_id="<user-id>")
    if error:
        print(error)
...
def example_get_user_personal_data():
    ...
    result, error = admin_get_user_personal_data_requests(user_id="<user-id>")
    if error:
        print(error)
...
def example_create_admin_email_configuration():
    ...
    result, error = save_admin_email_configuration(body=["foo"])
    if error:
        print(error)
...
def example_delete_admin_email_configuration():
    ...
    result, error = delete_admin_email_configuration(emails=["email@email.foo"])
    if error:
        print(error)
...
def example_update_admin_email_configuration():
    ...
    result, error = update_admin_email_configuration(body=["foo"])
    if error:
        print(error)
