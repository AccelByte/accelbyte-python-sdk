...
def example_create_leaderboard():
    ...
    result, error = create_leaderboard_configuration_admin_v1(
        body=ModelsLeaderboardConfigReq.create(
            daily=ModelsDailyConfig.create(reset_time="<reset-time>"),
            descending=True,
            icon_url="<icon-url>",
            leaderboard_code="<leaderboard_code>",
            monthly=ModelsMonthlyConfig.create(reset_date=1, reset_time="<reset-time>"),
            name="<name>",
            season_period=1,
            start_time="<start-time>",
            stat_code="<stat-code>",
            weekly=ModelsWeeklyConfig.create(reset_day=1, reset_time="<reset-time>"),
        )
    )
    if error:
        print(error)
