...
from accelbyte_py_sdk.api.leaderboard import (
    admin_get_archived_leaderboard_ranking_data_v1_handler as admin_get_archived_leaderboard_ranking_data_v1_handler_internal,
)
...
def admin_get_archived_leaderboard_ranking_data_v1_handler(
    ...
    if doc:
        click.echo(
            admin_get_archived_leaderboard_ranking_data_v1_handler_internal.__doc__
        )
        return
    ...
    if error:
        raise Exception(
            f"AdminGetArchivedLeaderboardRankingDataV1Handler failed: {str(error)}"
        )
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_get_archived_leaderboard_ranking_data_v1_handler.operation_id = (
