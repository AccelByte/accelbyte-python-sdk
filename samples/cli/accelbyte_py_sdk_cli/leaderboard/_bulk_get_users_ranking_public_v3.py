...
from accelbyte_py_sdk.api.leaderboard import (
    bulk_get_users_ranking_public_v3 as bulk_get_users_ranking_public_v3_internal,
)
...
def bulk_get_users_ranking_public_v3(
    ...
    if doc:
        click.echo(bulk_get_users_ranking_public_v3_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"bulkGetUsersRankingPublicV3 failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


bulk_get_users_ranking_public_v3.operation_id = "bulkGetUsersRankingPublicV3"
bulk_get_users_ranking_public_v3.is_deprecated = False
