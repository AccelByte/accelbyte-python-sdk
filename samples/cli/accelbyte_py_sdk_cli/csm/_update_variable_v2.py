...
from accelbyte_py_sdk.api.csm import update_variable_v2 as update_variable_v2_internal
...
def update_variable_v2(
    ...
    if doc:
        click.echo(update_variable_v2_internal.__doc__)
        return
    ...
    result, error = update_variable_v2_internal(
        body=body,
        app=app,
        config_id=config_id,
        namespace=namespace,
    ...
    if error:
        raise Exception(f"UpdateVariableV2 failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


update_variable_v2.operation_id = "UpdateVariableV2"
update_variable_v2.is_deprecated = False
