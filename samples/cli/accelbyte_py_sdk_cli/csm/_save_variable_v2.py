...
from accelbyte_py_sdk.api.csm import save_variable_v2 as save_variable_v2_internal
...
def save_variable_v2(
    ...
    if doc:
        click.echo(save_variable_v2_internal.__doc__)
        return
    ...
    result, error = save_variable_v2_internal(
        body=body,
        app=app,
        namespace=namespace,
    ...
    if error:
        raise Exception(f"SaveVariableV2 failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


save_variable_v2.operation_id = "SaveVariableV2"
save_variable_v2.is_deprecated = False
