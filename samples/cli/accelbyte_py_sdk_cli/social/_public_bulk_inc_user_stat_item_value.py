...
from accelbyte_py_sdk.api.social import public_bulk_inc_user_stat_item_value as public_bulk_inc_user_stat_item_value_internal
...
def public_bulk_inc_user_stat_item_value(
    ...
    _, error = public_bulk_inc_user_stat_item_value_internal(
        body=body,
        namespace=namespace,
    )
    if error:
        raise Exception(f"publicBulkIncUserStatItemValue failed: {str(error)}")
    click.echo("publicBulkIncUserStatItemValue success")
