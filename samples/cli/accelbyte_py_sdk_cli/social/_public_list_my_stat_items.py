...
from accelbyte_py_sdk.api.social import (
    public_list_my_stat_items as public_list_my_stat_items_internal,
)
...
def public_list_my_stat_items(
    ...
    if doc:
        click.echo(public_list_my_stat_items_internal.__doc__)
        return
    ...
    result, error = public_list_my_stat_items_internal(
        limit=limit,
        offset=offset,
        sort_by=sort_by,
    ...
    if error:
        raise Exception(f"publicListMyStatItems failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_list_my_stat_items.operation_id = "publicListMyStatItems"
public_list_my_stat_items.is_deprecated = False
