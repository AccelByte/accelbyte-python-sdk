...
from accelbyte_py_sdk.api.social import (
    admin_list_users_stat_items as admin_list_users_stat_items_internal,
)
...
def admin_list_users_stat_items(
    ...
    if doc:
        click.echo(admin_list_users_stat_items_internal.__doc__)
        return
    ...
    result, error = admin_list_users_stat_items_internal(
        user_id=user_id,
    ...
    if error:
        raise Exception(f"AdminListUsersStatItems failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_list_users_stat_items.operation_id = "AdminListUsersStatItems"
admin_list_users_stat_items.is_deprecated = False
