...
from accelbyte_py_sdk.api.social import (
    get_user_stat_cycle_items as get_user_stat_cycle_items_internal,
)
...
def get_user_stat_cycle_items(
    ...
    if doc:
        click.echo(get_user_stat_cycle_items_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"getUserStatCycleItems failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


get_user_stat_cycle_items.operation_id = "getUserStatCycleItems"
get_user_stat_cycle_items.is_deprecated = False
