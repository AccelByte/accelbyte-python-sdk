...
from accelbyte_py_sdk.api.challenge import (
    admin_get_challenge as admin_get_challenge_internal,
)
...
def admin_get_challenge(
    ...
    if doc:
        click.echo(admin_get_challenge_internal.__doc__)
        return
    ...
    result, error = admin_get_challenge_internal(
        challenge_code=challenge_code,
        namespace=namespace,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"adminGetChallenge failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_get_challenge.operation_id = "adminGetChallenge"
admin_get_challenge.is_deprecated = False
