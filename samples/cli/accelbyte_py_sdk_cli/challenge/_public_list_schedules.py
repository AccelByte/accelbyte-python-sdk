...
from accelbyte_py_sdk.api.challenge import (
    public_list_schedules as public_list_schedules_internal,
)
...
def public_list_schedules(
    ...
    if doc:
        click.echo(public_list_schedules_internal.__doc__)
        return
    ...
    result, error = public_list_schedules_internal(
    ...
    if error:
        raise Exception(f"publicListSchedules failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_list_schedules.operation_id = "publicListSchedules"
public_list_schedules.is_deprecated = False
