...
from accelbyte_py_sdk.api.challenge import (
    admin_evaluate_progress as admin_evaluate_progress_internal,
)
...
def admin_evaluate_progress(
    ...
    if doc:
        click.echo(admin_evaluate_progress_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"adminEvaluateProgress failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_evaluate_progress.operation_id = "adminEvaluateProgress"
admin_evaluate_progress.is_deprecated = False
