...
from accelbyte_py_sdk.api.challenge import (
    admin_get_periods as admin_get_periods_internal,
)
...
def admin_get_periods(
    ...
    if doc:
        click.echo(admin_get_periods_internal.__doc__)
        return
    ...
    result, error = admin_get_periods_internal(
    ...
    if error:
        raise Exception(f"adminGetPeriods failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_get_periods.operation_id = "adminGetPeriods"
admin_get_periods.is_deprecated = False
