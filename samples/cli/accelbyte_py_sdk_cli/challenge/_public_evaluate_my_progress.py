...
from accelbyte_py_sdk.api.challenge import (
    public_evaluate_my_progress as public_evaluate_my_progress_internal,
)
...
def public_evaluate_my_progress(
    ...
    if doc:
        click.echo(public_evaluate_my_progress_internal.__doc__)
        return
    ...
    result, error = public_evaluate_my_progress_internal(
    ...
    if error:
        raise Exception(f"publicEvaluateMyProgress failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_evaluate_my_progress.operation_id = "publicEvaluateMyProgress"
public_evaluate_my_progress.is_deprecated = False
