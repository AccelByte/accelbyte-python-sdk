...
from accelbyte_py_sdk.api.challenge import (
    admin_create_challenge as admin_create_challenge_internal,
)
...
def admin_create_challenge(
    ...
    if doc:
        click.echo(admin_create_challenge_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = ModelCreateChallengeRequest.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = admin_create_challenge_internal(
        body=body,
        namespace=namespace,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"adminCreateChallenge failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_create_challenge.operation_id = "adminCreateChallenge"
admin_create_challenge.is_deprecated = False
