...
from accelbyte_py_sdk.api.challenge import (
    admin_create_goal as admin_create_goal_internal,
)
...
def admin_create_goal(
    ...
    if doc:
        click.echo(admin_create_goal_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = ModelCreateGoalRequest.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = admin_create_goal_internal(
        body=body,
    ...
    if error:
        raise Exception(f"adminCreateGoal failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_create_goal.operation_id = "adminCreateGoal"
admin_create_goal.is_deprecated = False
