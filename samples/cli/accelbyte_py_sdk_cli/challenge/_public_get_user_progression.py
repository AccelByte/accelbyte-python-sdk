...
from accelbyte_py_sdk.api.challenge import (
    public_get_user_progression as public_get_user_progression_internal,
)
...
def public_get_user_progression(
    ...
    if doc:
        click.echo(public_get_user_progression_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"publicGetUserProgression failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_get_user_progression.operation_id = "publicGetUserProgression"
public_get_user_progression.is_deprecated = False
