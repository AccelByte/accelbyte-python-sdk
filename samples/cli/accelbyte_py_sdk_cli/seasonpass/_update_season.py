...
from accelbyte_py_sdk.api.seasonpass import update_season as update_season_internal
...
def update_season(
    ...
    if doc:
        click.echo(update_season_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = SeasonUpdate.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = update_season_internal(
        season_id=season_id,
        body=body,
        namespace=namespace,
