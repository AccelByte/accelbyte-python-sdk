...
from accelbyte_py_sdk.api.seasonpass import create_season as create_season_internal
...
def create_season(
    ...
    if doc:
        click.echo(create_season_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = SeasonCreate.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = create_season_internal(
        body=body,
        namespace=namespace,
