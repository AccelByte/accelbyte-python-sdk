...
from accelbyte_py_sdk.api.seasonpass import create_tier as create_tier_internal
...
def create_tier(
    ...
    if doc:
        click.echo(create_tier_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = TierCreate.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = create_tier_internal(
        season_id=season_id,
        body=body,
        namespace=namespace,
