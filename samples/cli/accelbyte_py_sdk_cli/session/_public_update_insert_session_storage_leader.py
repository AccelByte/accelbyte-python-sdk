...
def public_update_insert_session_storage_leader(
    ...
    if doc:
        click.echo(public_update_insert_session_storage_leader_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"publicUpdateInsertSessionStorageLeader failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_update_insert_session_storage_leader.operation_id = (
    "publicUpdateInsertSessionStorageLeader"
)
public_update_insert_session_storage_leader.is_deprecated = False
