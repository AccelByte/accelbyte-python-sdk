...
from accelbyte_py_sdk.api.matchmaking import (
    get_all_mock_tickets as get_all_mock_tickets_internal,
)
...
def get_all_mock_tickets(
    ...
    if doc:
        click.echo(get_all_mock_tickets_internal.__doc__)
        return
    ...
    result, error = get_all_mock_tickets_internal(
        channel_name=channel_name,
        namespace=namespace,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"GetAllMockTickets failed: {str(error)}")
