...
from accelbyte_py_sdk.api.matchmaking import (
    get_all_mock_matches as get_all_mock_matches_internal,
)
...
def get_all_mock_matches(
    ...
    if doc:
        click.echo(get_all_mock_matches_internal.__doc__)
        return
    ...
    result, error = get_all_mock_matches_internal(
        channel_name=channel_name,
        namespace=namespace,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"GetAllMockMatches failed: {str(error)}")
