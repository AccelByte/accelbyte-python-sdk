...
from accelbyte_py_sdk.api.matchmaking import (
    create_mock_tickets as create_mock_tickets_internal,
)
...
def create_mock_tickets(
    ...
    if doc:
        click.echo(create_mock_tickets_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = ModelsCreateMockTicket.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = create_mock_tickets_internal(
        body=body,
    ...
    if error:
        raise Exception(f"CreateMockTickets failed: {str(error)}")
