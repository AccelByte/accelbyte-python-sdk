...
from accelbyte_py_sdk.api.matchmaking import (
    get_mock_tickets_by_timestamp as get_mock_tickets_by_timestamp_internal,
)
...
def get_mock_tickets_by_timestamp(
    ...
    if doc:
        click.echo(get_mock_tickets_by_timestamp_internal.__doc__)
        return
    ...
    result, error = get_mock_tickets_by_timestamp_internal(
        body=body,
    ...
    if error:
        raise Exception(f"GetMockTicketsByTimestamp failed: {str(error)}")
