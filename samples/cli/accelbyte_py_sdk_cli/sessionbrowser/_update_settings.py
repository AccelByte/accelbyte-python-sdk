...
from accelbyte_py_sdk.api.sessionbrowser import update_settings as update_settings_internal
...
def update_settings(
    ...
    if doc:
        click.echo(update_settings_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = ModelsUpdateSettingsRequest.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = update_settings_internal(
        body=body,
    ...
    if error:
        raise Exception(f"UpdateSettings failed: {str(error)}")
