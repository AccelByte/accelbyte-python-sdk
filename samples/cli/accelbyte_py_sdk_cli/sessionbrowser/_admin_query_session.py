...
from accelbyte_py_sdk.api.sessionbrowser import (
    admin_query_session as admin_query_session_internal,
)
...
def admin_query_session(
    ...
    if doc:
        click.echo(admin_query_session_internal.__doc__)
        return
    ...
    result, error = admin_query_session_internal(
    ...
    if error:
        raise Exception(f"AdminQuerySession failed: {str(error)}")
