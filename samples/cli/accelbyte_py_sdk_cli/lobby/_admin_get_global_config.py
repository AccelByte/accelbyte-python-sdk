...
from accelbyte_py_sdk.api.lobby import (
    admin_get_global_config as admin_get_global_config_internal,
)
...
def admin_get_global_config(
    ...
    if doc:
        click.echo(admin_get_global_config_internal.__doc__)
        return
    ...
    result, error = admin_get_global_config_internal(
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"adminGetGlobalConfig failed: {str(error)}")
