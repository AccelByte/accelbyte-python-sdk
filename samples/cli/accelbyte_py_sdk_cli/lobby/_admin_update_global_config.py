...
from accelbyte_py_sdk.api.lobby import (
    admin_update_global_config as admin_update_global_config_internal,
)
...
def admin_update_global_config(
    ...
    if doc:
        click.echo(admin_update_global_config_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = ModelPutGlobalConfigurationRequest.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = admin_update_global_config_internal(
        body=body,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"adminUpdateGlobalConfig failed: {str(error)}")
