...
from accelbyte_py_sdk.api.lobby import (
    public_update_party_attributes_v1 as public_update_party_attributes_v1_internal,
)
...
def public_update_party_attributes_v1(
    ...
    if doc:
        click.echo(public_update_party_attributes_v1_internal.__doc__)
        return
    ...
    result, error = public_update_party_attributes_v1_internal(
        body=body,
        party_id=party_id,
        namespace=namespace,
    ...
    if error:
        raise Exception(f"publicUpdatePartyAttributesV1 failed: {str(error)}")
