...
from accelbyte_py_sdk.api.achievement import (
    list_user_contributions as list_user_contributions_internal,
)
...
def list_user_contributions(
    ...
    if doc:
        click.echo(list_user_contributions_internal.__doc__)
        return
    ...
    result, error = list_user_contributions_internal(
        user_id=user_id,
    ...
    if error:
        raise Exception(f"ListUserContributions failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


list_user_contributions.operation_id = "ListUserContributions"
list_user_contributions.is_deprecated = False
