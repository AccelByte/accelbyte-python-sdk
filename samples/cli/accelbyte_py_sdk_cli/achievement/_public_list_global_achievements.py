...
from accelbyte_py_sdk.api.achievement import (
    public_list_global_achievements as public_list_global_achievements_internal,
)
...
def public_list_global_achievements(
    ...
    if doc:
        click.echo(public_list_global_achievements_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"PublicListGlobalAchievements failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_list_global_achievements.operation_id = "PublicListGlobalAchievements"
public_list_global_achievements.is_deprecated = False
