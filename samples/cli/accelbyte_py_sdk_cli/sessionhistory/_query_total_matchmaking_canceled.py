...
from accelbyte_py_sdk.api.sessionhistory import (
    query_total_matchmaking_canceled as query_total_matchmaking_canceled_internal,
)
...
def query_total_matchmaking_canceled(
    ...
    if doc:
        click.echo(query_total_matchmaking_canceled_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"queryTotalMatchmakingCanceled failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_total_matchmaking_canceled.operation_id = "queryTotalMatchmakingCanceled"
query_total_matchmaking_canceled.is_deprecated = False
