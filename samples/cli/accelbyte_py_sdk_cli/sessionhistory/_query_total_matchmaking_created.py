...
from accelbyte_py_sdk.api.sessionhistory import (
    query_total_matchmaking_created as query_total_matchmaking_created_internal,
)
...
def query_total_matchmaking_created(
    ...
    if doc:
        click.echo(query_total_matchmaking_created_internal.__doc__)
        return
    ...
    result, error = query_total_matchmaking_created_internal(
        end_date=end_date,
    ...
    if error:
        raise Exception(f"queryTotalMatchmakingCreated failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_total_matchmaking_created.operation_id = "queryTotalMatchmakingCreated"
query_total_matchmaking_created.is_deprecated = False
