...
from accelbyte_py_sdk.api.sessionhistory import (
    query_xray_match as query_xray_match_internal,
)
...
def query_xray_match(
    ...
    if doc:
        click.echo(query_xray_match_internal.__doc__)
        return
    ...
    result, error = query_xray_match_internal(
        match_id=match_id,
        namespace=namespace,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"queryXrayMatch failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_xray_match.operation_id = "queryXrayMatch"
query_xray_match.is_deprecated = False
