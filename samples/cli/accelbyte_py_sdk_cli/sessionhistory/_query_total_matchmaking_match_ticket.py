...
from accelbyte_py_sdk.api.sessionhistory import (
    query_total_matchmaking_match_ticket as query_total_matchmaking_match_ticket_internal,
)
...
def query_total_matchmaking_match_ticket(
    ...
    if doc:
        click.echo(query_total_matchmaking_match_ticket_internal.__doc__)
        return
    ...
    result, error = query_total_matchmaking_match_ticket_internal(
        end_date=end_date,
    ...
    if error:
        raise Exception(f"queryTotalMatchmakingMatchTicket failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_total_matchmaking_match_ticket.operation_id = "queryTotalMatchmakingMatchTicket"
query_total_matchmaking_match_ticket.is_deprecated = False
