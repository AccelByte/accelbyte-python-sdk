...
from accelbyte_py_sdk.api.sessionhistory import (
    admin_query_party_detail as admin_query_party_detail_internal,
)
...
def admin_query_party_detail(
    ...
    if doc:
        click.echo(admin_query_party_detail_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"adminQueryPartyDetail failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_query_party_detail.operation_id = "adminQueryPartyDetail"
admin_query_party_detail.is_deprecated = False
