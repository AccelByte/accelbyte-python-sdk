...
from accelbyte_py_sdk.api.sessionhistory import (
    query_acquiring_ds as query_acquiring_ds_internal,
)
...
def query_acquiring_ds(
    ...
    if doc:
        click.echo(query_acquiring_ds_internal.__doc__)
        return
    ...
    result, error = query_acquiring_ds_internal(
        end_date=end_date,
    ...
    if error:
        raise Exception(f"queryAcquiringDS failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_acquiring_ds.operation_id = "queryAcquiringDS"
query_acquiring_ds.is_deprecated = False
