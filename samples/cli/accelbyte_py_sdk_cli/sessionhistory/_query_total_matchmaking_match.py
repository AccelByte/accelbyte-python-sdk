...
from accelbyte_py_sdk.api.sessionhistory import (
    query_total_matchmaking_match as query_total_matchmaking_match_internal,
)
...
def query_total_matchmaking_match(
    ...
    if doc:
        click.echo(query_total_matchmaking_match_internal.__doc__)
        return
    ...
    result, error = query_total_matchmaking_match_internal(
        end_date=end_date,
    ...
    if error:
        raise Exception(f"queryTotalMatchmakingMatch failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_total_matchmaking_match.operation_id = "queryTotalMatchmakingMatch"
query_total_matchmaking_match.is_deprecated = False
