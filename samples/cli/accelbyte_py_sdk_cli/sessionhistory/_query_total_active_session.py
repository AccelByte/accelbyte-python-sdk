...
from accelbyte_py_sdk.api.sessionhistory import (
    query_total_active_session as query_total_active_session_internal,
)
...
def query_total_active_session(
    ...
    if doc:
        click.echo(query_total_active_session_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"queryTotalActiveSession failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_total_active_session.operation_id = "queryTotalActiveSession"
query_total_active_session.is_deprecated = False
