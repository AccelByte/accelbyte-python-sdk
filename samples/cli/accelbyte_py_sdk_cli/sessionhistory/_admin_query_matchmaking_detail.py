...
from accelbyte_py_sdk.api.sessionhistory import (
    admin_query_matchmaking_detail as admin_query_matchmaking_detail_internal,
)
...
@click.option("--ticket_id", "ticket_id", type=str)
...
def admin_query_matchmaking_detail(
    ...
    if doc:
        click.echo(admin_query_matchmaking_detail_internal.__doc__)
        return
    ...
    result, error = admin_query_matchmaking_detail_internal(
    ...
    if error:
        raise Exception(f"adminQueryMatchmakingDetail failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_query_matchmaking_detail.operation_id = "adminQueryMatchmakingDetail"
admin_query_matchmaking_detail.is_deprecated = False
