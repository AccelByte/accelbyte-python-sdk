...
from accelbyte_py_sdk.api.sessionhistory import (
    query_match_length_durationp_avg as query_match_length_durationp_avg_internal,
)
...
def query_match_length_durationp_avg(
    ...
    if doc:
        click.echo(query_match_length_durationp_avg_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"queryMatchLengthDurationpAvg failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


query_match_length_durationp_avg.operation_id = "queryMatchLengthDurationpAvg"
query_match_length_durationp_avg.is_deprecated = False
