...
from accelbyte_py_sdk.api.inventory import (
    admin_update_inventory as admin_update_inventory_internal,
)
...
def admin_update_inventory(
    ...
    if doc:
        click.echo(admin_update_inventory_internal.__doc__)
        return
    ...
    result, error = admin_update_inventory_internal(
        body=body,
    ...
    if error:
        raise Exception(f"AdminUpdateInventory failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_update_inventory.operation_id = "AdminUpdateInventory"
admin_update_inventory.is_deprecated = False
