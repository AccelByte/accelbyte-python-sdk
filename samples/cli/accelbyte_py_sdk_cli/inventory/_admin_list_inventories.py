...
from accelbyte_py_sdk.api.inventory import (
    admin_list_inventories as admin_list_inventories_internal,
)
...
def admin_list_inventories(
    ...
    if doc:
        click.echo(admin_list_inventories_internal.__doc__)
        return
    ...
    result, error = admin_list_inventories_internal(
    ...
    if error:
        raise Exception(f"AdminListInventories failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_list_inventories.operation_id = "AdminListInventories"
admin_list_inventories.is_deprecated = False
