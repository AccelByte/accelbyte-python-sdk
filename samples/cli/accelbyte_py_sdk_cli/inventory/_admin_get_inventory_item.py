...
from accelbyte_py_sdk.api.inventory import (
    admin_get_inventory_item as admin_get_inventory_item_internal,
)
...
def admin_get_inventory_item(
    ...
    if doc:
        click.echo(admin_get_inventory_item_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"AdminGetInventoryItem failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_get_inventory_item.operation_id = "AdminGetInventoryItem"
admin_get_inventory_item.is_deprecated = False
