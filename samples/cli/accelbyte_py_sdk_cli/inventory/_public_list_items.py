...
from accelbyte_py_sdk.api.inventory import (
    public_list_items as public_list_items_internal,
)
...
def public_list_items(
    ...
    if doc:
        click.echo(public_list_items_internal.__doc__)
        return
    ...
    result, error = public_list_items_internal(
    ...
    if error:
        raise Exception(f"PublicListItems failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_list_items.operation_id = "PublicListItems"
public_list_items.is_deprecated = False
