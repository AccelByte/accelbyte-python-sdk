...
from accelbyte_py_sdk.api.inventory import (
    public_consume_my_item as public_consume_my_item_internal,
)
...
def public_consume_my_item(
    ...
    if doc:
        click.echo(public_consume_my_item_internal.__doc__)
        return
    ...
    result, error = public_consume_my_item_internal(
        body=body,
    ...
    if error:
        raise Exception(f"PublicConsumeMyItem failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_consume_my_item.operation_id = "PublicConsumeMyItem"
public_consume_my_item.is_deprecated = False
