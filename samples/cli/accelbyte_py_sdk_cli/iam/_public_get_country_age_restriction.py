...
from accelbyte_py_sdk.api.iam import public_get_country_age_restriction as public_get_country_age_restriction_internal
...
def public_get_country_age_restriction(
    ...
    if doc:
        click.echo(public_get_country_age_restriction_internal.__doc__)
        return
    ...
    result, error = public_get_country_age_restriction_internal(
        country_code=country_code,
        namespace=namespace,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"PublicGetCountryAgeRestriction failed: {str(error)}")
