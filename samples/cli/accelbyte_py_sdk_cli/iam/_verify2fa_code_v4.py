...
from accelbyte_py_sdk.api.iam import verify2fa_code_v4 as verify2fa_code_v4_internal
...
def verify2fa_code_v4(
    ...
    if doc:
        click.echo(verify2fa_code_v4_internal.__doc__)
        return
    ...
    result, error = verify2fa_code_v4_internal(
        code=code,
        factor=factor,
        mfa_token=mfa_token,
    ...
    if error:
        raise Exception(f"Verify2FACodeV4 failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


verify2fa_code_v4.operation_id = "Verify2FACodeV4"
verify2fa_code_v4.is_deprecated = False
