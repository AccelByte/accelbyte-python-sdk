...
from accelbyte_py_sdk.api.iam import (
    admin_create_user_v4 as admin_create_user_v4_internal,
)
...
def admin_create_user_v4(
    ...
    if doc:
        click.echo(admin_create_user_v4_internal.__doc__)
        return
    ...
    result, error = admin_create_user_v4_internal(
        body=body,
        namespace=namespace,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"AdminCreateUserV4 failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


admin_create_user_v4.operation_id = "AdminCreateUserV4"
admin_create_user_v4.is_deprecated = False
