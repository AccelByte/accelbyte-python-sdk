...
from accelbyte_py_sdk.api.iam import (
    retrieve_third_party_login_platform_credential_v3 as retrieve_third_party_login_platform_credential_v3_internal,
)
...
def retrieve_third_party_login_platform_credential_v3(
    ...
    if error:
        raise Exception(
            f"RetrieveThirdPartyLoginPlatformCredentialV3 failed: {str(error)}"
        )
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


retrieve_third_party_login_platform_credential_v3.operation_id = (
    "RetrieveThirdPartyLoginPlatformCredentialV3"
)
retrieve_third_party_login_platform_credential_v3.is_deprecated = False
