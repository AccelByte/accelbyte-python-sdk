...
def public_upgrade_headless_with_code_v4_forward(
    ...
    if doc:
        click.echo(public_upgrade_headless_with_code_v4_forward_internal.__doc__)
        return
    ...
    result, error = public_upgrade_headless_with_code_v4_forward_internal(
        body=body,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"PublicUpgradeHeadlessWithCodeV4Forward failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


public_upgrade_headless_with_code_v4_forward.operation_id = (
    "PublicUpgradeHeadlessWithCodeV4Forward"
)
public_upgrade_headless_with_code_v4_forward.is_deprecated = False
