...
from accelbyte_py_sdk.api.iam import (
    simultaneous_login_v3 as simultaneous_login_v3_internal,
)
...
def simultaneous_login_v3(
    ...
    if doc:
        click.echo(simultaneous_login_v3_internal.__doc__)
        return
    ...
    result, error = simultaneous_login_v3_internal(
    ...
    if error:
        raise Exception(f"SimultaneousLoginV3 failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


simultaneous_login_v3.operation_id = "SimultaneousLoginV3"
simultaneous_login_v3.is_deprecated = False
