...
from accelbyte_py_sdk.api.iam import update_role as update_role_internal
...
def update_role(
    ...
    try:
        ...
        body = ModelRoleUpdateRequest.create_from_dict(body_json)
    except ValueError as e:
        raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    _, error = update_role_internal(
        body=body,
        role_id=role_id,
    )
    if error:
        raise Exception(f"UpdateRole failed: {str(error)}")
    click.echo("UpdateRole success")
