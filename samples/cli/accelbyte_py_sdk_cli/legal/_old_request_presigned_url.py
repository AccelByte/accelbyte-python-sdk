...
from accelbyte_py_sdk.api.legal import (
    old_request_presigned_url as old_request_presigned_url_internal,
)
...
def old_request_presigned_url(
    ...
    if doc:
        click.echo(old_request_presigned_url_internal.__doc__)
        return
    ...
    result, error = old_request_presigned_url_internal(
        localized_policy_version_id=localized_policy_version_id,
        body=body,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldRequestPresignedURL failed: {str(error)}")
