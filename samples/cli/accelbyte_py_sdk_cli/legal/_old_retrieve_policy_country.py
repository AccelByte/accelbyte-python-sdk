...
from accelbyte_py_sdk.api.legal import (
    old_retrieve_policy_country as old_retrieve_policy_country_internal,
)
...
def old_retrieve_policy_country(
    ...
    if doc:
        click.echo(old_retrieve_policy_country_internal.__doc__)
        return
    ...
    result, error = old_retrieve_policy_country_internal(
        base_policy_id=base_policy_id,
        country_code=country_code,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldRetrievePolicyCountry failed: {str(error)}")
