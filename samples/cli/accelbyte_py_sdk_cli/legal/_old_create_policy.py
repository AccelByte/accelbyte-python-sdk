...
from accelbyte_py_sdk.api.legal import old_create_policy as old_create_policy_internal
...
def old_create_policy(
    ...
    if doc:
        click.echo(old_create_policy_internal.__doc__)
        return
    ...
    result, error = old_create_policy_internal(
        body=body,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldCreatePolicy failed: {str(error)}")
