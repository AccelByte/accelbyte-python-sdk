...
from accelbyte_py_sdk.api.legal import (
    request_presigned_url_1 as request_presigned_url_1_internal,
)
...
def request_presigned_url_1(
    ...
    if doc:
        click.echo(request_presigned_url_1_internal.__doc__)
        return
    ...
    result, error = request_presigned_url_1_internal(
    ...
    if error:
        raise Exception(f"requestPresignedURL_1 failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


request_presigned_url_1.operation_id = "requestPresignedURL_1"
request_presigned_url_1.is_deprecated = False
