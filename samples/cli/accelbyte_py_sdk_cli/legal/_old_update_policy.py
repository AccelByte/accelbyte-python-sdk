...
from accelbyte_py_sdk.api.legal import old_update_policy as old_update_policy_internal
...
def old_update_policy(
    ...
    if doc:
        click.echo(old_update_policy_internal.__doc__)
        return
    ...
    result, error = old_update_policy_internal(
        policy_id=policy_id,
        body=body,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldUpdatePolicy failed: {str(error)}")
