...
from accelbyte_py_sdk.api.legal import (
    old_retrieve_accepted_agreements as old_retrieve_accepted_agreements_internal,
)
...
def old_retrieve_accepted_agreements(
    ...
    if doc:
        click.echo(old_retrieve_accepted_agreements_internal.__doc__)
        return
    ...
    result, error = old_retrieve_accepted_agreements_internal(
        user_id=user_id,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldRetrieveAcceptedAgreements failed: {str(error)}")
