...
from accelbyte_py_sdk.api.legal import (
    old_partial_update_policy as old_partial_update_policy_internal,
)
...
def old_partial_update_policy(
    ...
    if doc:
        click.echo(old_partial_update_policy_internal.__doc__)
        return
    ...
    result, error = old_partial_update_policy_internal(
        base_policy_id=base_policy_id,
        body=body,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldPartialUpdatePolicy failed: {str(error)}")
