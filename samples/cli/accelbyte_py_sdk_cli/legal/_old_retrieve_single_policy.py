...
from accelbyte_py_sdk.api.legal import (
    old_retrieve_single_policy as old_retrieve_single_policy_internal,
)
...
def old_retrieve_single_policy(
    ...
    if doc:
        click.echo(old_retrieve_single_policy_internal.__doc__)
        return
    ...
    result, error = old_retrieve_single_policy_internal(
        base_policy_id=base_policy_id,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldRetrieveSinglePolicy failed: {str(error)}")
