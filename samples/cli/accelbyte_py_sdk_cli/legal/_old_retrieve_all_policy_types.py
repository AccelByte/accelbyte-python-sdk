...
from accelbyte_py_sdk.api.legal import (
    old_retrieve_all_policy_types as old_retrieve_all_policy_types_internal,
)
...
def old_retrieve_all_policy_types(
    ...
    if doc:
        click.echo(old_retrieve_all_policy_types_internal.__doc__)
        return
    ...
    result, error = old_retrieve_all_policy_types_internal(
        limit=limit,
        offset=offset,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldRetrieveAllPolicyTypes failed: {str(error)}")
