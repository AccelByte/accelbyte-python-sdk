...
from accelbyte_py_sdk.api.legal import (
    old_retrieve_localized_policy_versions as old_retrieve_localized_policy_versions_internal,
)
...
def old_retrieve_localized_policy_versions(
    ...
    if doc:
        click.echo(old_retrieve_localized_policy_versions_internal.__doc__)
        return
    ...
    result, error = old_retrieve_localized_policy_versions_internal(
        policy_version_id=policy_version_id,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldRetrieveLocalizedPolicyVersions failed: {str(error)}")
