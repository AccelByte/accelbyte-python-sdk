...
from accelbyte_py_sdk.api.legal import (
    old_publish_policy_version as old_publish_policy_version_internal,
)
...
def old_publish_policy_version(
    ...
    if doc:
        click.echo(old_publish_policy_version_internal.__doc__)
        return
    ...
    result, error = old_publish_policy_version_internal(
        policy_version_id=policy_version_id,
        should_notify=should_notify,
        x_additional_headers=x_additional_headers,
    )
    if error:
        raise Exception(f"oldPublishPolicyVersion failed: {str(error)}")
