...
def check_balance(
    ...
    if doc:
        click.echo(check_balance_internal.__doc__)
        return
    ...
    result, error = check_balance_internal(
        request=request,
    ...
    if error:
        raise Exception(f"checkBalance failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


check_balance.operation_id = "checkBalance"
check_balance.is_deprecated = False
