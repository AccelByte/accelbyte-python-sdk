...
from accelbyte_py_sdk.api.platform import get_stadia_iap_config as get_stadia_iap_config_internal
...
def get_stadia_iap_config(
    ...
    _, error = get_stadia_iap_config_internal(
        namespace=namespace,
    )
    if error:
        raise Exception(f"getStadiaIAPConfig failed: {str(error)}")
    click.echo("getStadiaIAPConfig success")
