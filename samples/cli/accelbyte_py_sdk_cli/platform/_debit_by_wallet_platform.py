...
from accelbyte_py_sdk.api.platform import (
    debit_by_wallet_platform as debit_by_wallet_platform_internal,
)
...
def debit_by_wallet_platform(
    ...
    if doc:
        click.echo(debit_by_wallet_platform_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"debitByWalletPlatform failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


debit_by_wallet_platform.operation_id = "debitByWalletPlatform"
debit_by_wallet_platform.is_deprecated = False
