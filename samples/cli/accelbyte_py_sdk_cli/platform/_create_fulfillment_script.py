...
from accelbyte_py_sdk.api.platform import (
    create_fulfillment_script as create_fulfillment_script_internal,
)
...
def create_fulfillment_script(
    ...
    if doc:
        click.echo(create_fulfillment_script_internal.__doc__)
        return
    ...
    if body is not None:
        try:
            ...
            body = FulfillmentScriptCreate.create_from_dict(body_json)
        except ValueError as e:
            raise Exception(f"Invalid JSON for 'body'. {str(e)}") from e
    result, error = create_fulfillment_script_internal(
        id_=id_,
        body=body,
