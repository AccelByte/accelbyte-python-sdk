...
from accelbyte_py_sdk.api.platform import refund_user_payment_order as refund_user_payment_order_internal
...
def refund_user_payment_order(
    ...
    if doc:
        click.echo(refund_user_payment_order_internal.__doc__)
        return
    ...
    if error:
        raise Exception(f"refundUserPaymentOrder failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


refund_user_payment_order.operation_id = "refundUserPaymentOrder"
refund_user_payment_order.is_deprecated = False
