...
from accelbyte_py_sdk.api.dsartifact import list_queue as list_queue_internal
...
def list_queue(
    ...
    if doc:
        click.echo(list_queue_internal.__doc__)
        return
    ...
    result, error = list_queue_internal(
        node_ip=node_ip,
        limit=limit,
        next_=next_,
        previous=previous,
    ...
    if error:
        raise Exception(f"listQueue failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


list_queue.operation_id = "listQueue"
list_queue.is_deprecated = False
