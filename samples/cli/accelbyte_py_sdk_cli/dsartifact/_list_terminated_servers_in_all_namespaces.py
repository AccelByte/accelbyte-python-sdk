...
from accelbyte_py_sdk.api.dsartifact import (
    list_terminated_servers_in_all_namespaces as list_terminated_servers_in_all_namespaces_internal,
)
...
def list_terminated_servers_in_all_namespaces(
    ...
    if doc:
        click.echo(list_terminated_servers_in_all_namespaces_internal.__doc__)
        return
    ...
    result, error = list_terminated_servers_in_all_namespaces_internal(
    ...
    if error:
        raise Exception(f"listTerminatedServersInAllNamespaces failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


list_terminated_servers_in_all_namespaces.operation_id = (
    "listTerminatedServersInAllNamespaces"
)
list_terminated_servers_in_all_namespaces.is_deprecated = False
