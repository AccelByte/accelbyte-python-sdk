...
from accelbyte_py_sdk.api.dsartifact import (
    list_all_active_queue as list_all_active_queue_internal,
)
...
def list_all_active_queue(
    ...
    if doc:
        click.echo(list_all_active_queue_internal.__doc__)
        return
    ...
    result, error = list_all_active_queue_internal(
        limit=limit,
        next_=next_,
        node_ip=node_ip,
        pod_name=pod_name,
    ...
    if error:
        raise Exception(f"listAllActiveQueue failed: {str(error)}")
    click.echo(yaml.safe_dump(to_dict(result), sort_keys=False))


list_all_active_queue.operation_id = "listAllActiveQueue"
list_all_active_queue.is_deprecated = False
